<template>
  <section id="regionen" class="py-20 bg-base-200">
    <div class="container mx-auto px-4">
      <div class="text-center mb-16">
        <h3 class="text-lg text-[#f60] font-semibold" data-aos="fade-up">
          Ihr lokaler Google Ads Partner
        </h3>
        <h2
          class="text-4xl text-white font-bold"
          data-aos="fade-up"
          data-aos-delay="100"
        >
          Google Ads Betreuung in Ihrer Region
        </h2>
        <p
          class="mt-4 text-lg text-white max-w-3xl mx-auto"
          data-aos="fade-up"
          data-aos-delay="200"
        >
          Wir betreuen Google Ads Kampagnen für Unternehmen in Bad Oeynhausen,
          Herford, Minden und Umgebung. Mit lokaler Expertise und persönlicher
          Beratung vor Ort.
        </p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <!-- Bad Oeynhausen -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Bad Oeynhausen</h3>
            </div>
            <p class="text-white mb-6">
              Als Google Ads Agentur aus Bad Oeynhausen kennen wir den lokalen
              Markt und erstellen maßgeschneiderte Kampagnen, die gezielt Kunden
              aus Ihrer Region ansprechen und Ihren ROI maximieren.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Unsere Leistungen:</h4>
              <ul class="space-y-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Bad Oeynhausen
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  SEA Bad Oeynhausen
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Kampagnen erstellen
                </li>
              </ul>
            </div>
          </div>
        </div>

        <!-- Herford -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
          data-aos-delay="100"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Herford</h3>
            </div>
            <p class="text-white mb-6">
              Für Unternehmen aus Herford entwickeln wir effektive Google Ads
              Strategien, die Ihre Sichtbarkeit erhöhen und qualifizierte Leads
              generieren. Mit persönlicher Betreuung vor Ort.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Unsere Leistungen:</h4>
              <ul class="space-y-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Agentur Herford
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads optimieren Herford
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Beratung Herford
                </li>
              </ul>
            </div>
          </div>
        </div>

        <!-- Minden -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
          data-aos-delay="200"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Minden</h3>
            </div>
            <p class="text-white mb-6">
              Wir unterstützen Unternehmen aus Minden mit professionellen Google
              Ads Kampagnen, die gezielt auf Ihre Zielgruppe ausgerichtet sind
              und messbare Ergebnisse liefern.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Unsere Leistungen:</h4>
              <ul class="space-y-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  SEA Agentur Minden
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Minden
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Kampagnen Minden
                </li>
              </ul>
            </div>
          </div>
        </div>

        <!-- Porta Westfalica -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
          data-aos-delay="300"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Porta Westfalica</h3>
            </div>
            <p class="text-white mb-6">
              Für Unternehmen aus Porta Westfalica bieten wir maßgeschneiderte
              Google Ads Lösungen, die Ihren Umsatz steigern und Ihre
              Marketingkosten optimieren.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Unsere Leistungen:</h4>
              <ul class="space-y-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Porta Westfalica
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads einrichten Porta Westfalica
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  SEA Porta Westfalica
                </li>
              </ul>
            </div>
          </div>
        </div>

        <!-- Bünde -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
          data-aos-delay="400"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Bünde</h3>
            </div>
            <p class="text-white mb-6">
              Wir erstellen für Unternehmen aus Bünde erfolgreiche Google Ads
              Kampagnen, die gezielt neue Kunden ansprechen und Ihren Return on
              Investment maximieren.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Unsere Leistungen:</h4>
              <ul class="space-y-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads Bünde
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Google Ads optimieren Bünde
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  SEA Beratung Bünde
                </li>
              </ul>
            </div>
          </div>
        </div>

        <!-- Weitere Regionen -->
        <div
          class="card bg-black shadow-xl border border-[#f60]/20 hover:border-[#f60] transition-all duration-300"
          data-aos="fade-up"
          data-aos-delay="500"
        >
          <div class="card-body">
            <div class="flex items-center mb-4">
              <div
                class="w-12 h-12 bg-[#f60]/10 rounded-lg flex items-center justify-center mr-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#f60"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                >
                  <circle cx="12" cy="12" r="10" />
                  <path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20" />
                  <path d="M2 12h20" />
                </svg>
              </div>
              <h3 class="text-2xl font-bold text-white">Weitere Regionen</h3>
            </div>
            <p class="text-white mb-6">
              Wir betreuen Google Ads Kampagnen für Kunden im gesamten Kreis
              Minden-Lübbecke, Herford und darüber hinaus. Profitieren Sie von
              unserer Expertise und persönlichen Betreuung.
            </p>
            <div class="mt-auto">
              <h4 class="font-semibold text-[#f60] mb-2">Weitere Standorte:</h4>
              <ul class="grid grid-cols-2 gap-1">
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Löhne
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Bielefeld
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Espelkamp
                </li>
                <li class="flex items-center text-white">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="#f60"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    class="mr-2"
                  >
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                  Lübbecke
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>

      <!-- Regionale Karte -->
      <div
        class="mt-16 bg-black p-6 rounded-lg border border-[#f60]/20"
        data-aos="fade-up"
      >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            <h3 class="text-2xl font-bold text-white mb-4">
              Ihr lokaler Google Ads Experte in Ostwestfalen
            </h3>
            <p class="text-white mb-4">
              Als Google Ads Agentur aus Bad Oeynhausen kennen wir die lokalen
              Märkte und Zielgruppen in Ostwestfalen. Wir wissen genau, wie Sie
              Ihre Werbeanzeigen optimal auf regionale Kunden ausrichten können.
            </p>
            <p class="text-white mb-6">
              Unsere Kunden schätzen die Kombination aus digitalem Know-how und
              persönlicher Betreuung vor Ort. Wir sind nicht nur online für Sie
              da, sondern auch persönlich ansprechbar.
            </p>
            <button
              class="btn bg-[#f60] text-white border-none hover:bg-[#ff751a]"
              onclick="window.open('https://zeeg.me/orangize', '_blank')"
            >
              Kostenlose Google Ads Beratung
            </button>
          </div>
          <div class="relative h-64 md:h-auto rounded-lg overflow-hidden">
            <img
              src="@/assets/images/ostwestfalen-karte.png"
              alt="Karte von Ostwestfalen mit Bad Oeynhausen, Herford und Minden"
              class="w-full h-full object-cover rounded-lg"
            />
            <div
              class="absolute inset-0 bg-gradient-to-t from-black to-transparent opacity-60"
            ></div>
            <div
              class="absolute bottom-4 left-4 right-4 text-white text-center"
            >
              <p class="text-sm">
                Google Ads Betreuung in Bad Oeynhausen, Herford, Minden und
                Umgebung
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
import { onMounted } from "vue";

// Wenn Sie AOS (Animate on Scroll) verwenden, können Sie es hier initialisieren
onMounted(() => {
  if (window.AOS) {
    window.AOS.refresh();
  }
});
</script>

<style scoped>
.card {
  transition: transform 0.3s ease, box-shadow 0.3s ease, border-color 0.3s ease;
  height: 100%;
  display: flex;
  flex-direction: column;
}

.card:hover {
  transform: translateY(-5px);
  box-shadow: 0 10px 25px rgba(255, 102, 0, 0.2);
}

.card-body {
  display: flex;
  flex-direction: column;
  height: 100%;
}
</style>
