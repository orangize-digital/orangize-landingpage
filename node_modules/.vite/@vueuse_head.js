...
function resolvePackedMetaObjectValue(value, key) {
  ...
  return unpackToString(changeKeyCasingDeep(value), __spreadValues({
    keyValueSeparator: "=",
    entrySeparator: ", ",
...
function handleObjectEntry(key, v) {
  ...
  return [__spreadValues({ [attr]: fKey }, value)];
}
...
function normaliseTag(tagName, input, e, normalizedProps) {
  const props = normalizedProps || normaliseProps(typeof input === "object" && typeof input !== "function" && !(input instanceof Promise) ? __spreadValues({}, input) : { [tagName === "script" || tagName === "noscript" || tagName === "style" ? "innerHTML" : "textContent"]: input }, tagName === "templateParams" || tagName === "titleTemplate");
  ...
  return Array.isArray(tag.props.content) ? tag.props.content.map((v) => __spreadProps(__spreadValues({}, tag), { props: __spreadProps(__spreadValues({}, tag.props), { content: v }) })) : tag;
}
...
async function renderDOMHead(head, options = {}) {
  ...
  head._domUpdatePromise = new Promise(async (resolve) => {
    ...
    if (!state) {
      ...
      for (const key of ["body", "head"]) {
        ...
        for (const c of children) {
          ...
          const t = {
            tag,
            props: await normaliseProps(c.getAttributeNames().reduce((props, name) => __spreadProps(__spreadValues({}, props), { [name]: c.getAttribute(name) }), {})),
    ...
    state.pendingSideEffects = __spreadValues({}, state.sideEffects);
    state.sideEffects = {};
      ...
      for (const tag of ctx.tags) {
        ...
        if (dupedTag) {
          ...
          if (strategy === "merge") {
            ...
            deduping[dedupeKey].props = __spreadValues(__spreadValues({}, oldProps), tag.props);
            continue;
...
var TemplateParamsPlugin = defineHeadPlugin((head) => ({
  hooks: {
    "tags:resolve": (ctx) => {
      ...
      params.pageTitle = processTemplateParams(params.pageTitle || ((_a = tags.find((tag) => tag.tag === "title")) == null ? void 0 : _a.textContent) || "", params, sep);
...
function useHeadSafe(input, options = {}) {
  return useHead(input, __spreadProps(__spreadValues({}, options), { transform: whitelistSafeInput }));
}
function useSeoMeta(input, options) {
  const _a = input, { title, titleTemplate } = _a, meta = __objRest(_a, ["title", "titleTemplate"]);
  return useHead({
    title,
    titleTemplate,
    _flatMeta: meta
    ...
    transform(t) {
      const meta2 = unpackMeta(__spreadValues({}, t._flatMeta));
      delete t._flatMeta;
...
function useServerHead(input, options = {}) {
  ...
  delete options.head;
  if (head)
    return head.push(input, __spreadProps(__spreadValues({}, options), { mode: "server" }));
}
function useServerHeadSafe(input, options = {}) {
  return useHeadSafe(input, __spreadProps(__spreadValues({}, options), { mode: "server" }));
}
function useServerSeoMeta(input, options) {
  return useSeoMeta(input, __spreadProps(__spreadValues({}, options), { mode: "server" }));
}
  ...
  _Vue.mixin({
    beforeCreate() {
      ...
      options.provide = function() {
        ...
        return __spreadProps(__spreadValues({}, origProvideResult), {
          [headSymbol]: head
        });
      };
    }
